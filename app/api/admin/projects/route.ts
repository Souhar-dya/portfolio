...
export async function GET() {
  try {
    ...
    const projects = await dbService.getProjects(true) // Include hidden projects for admin

    return NextResponse.json({ projects })
  } catch (error) {
    console.error("Admin projects fetch error:", error)

    // Return sample projects for admin if database is not connected
    const sampleProjects = [
      ...
      },
      {
        ...
        description: "This is a hidden project that won't show on the main portfolio",
        category: "backend",
        githubUrl: "https://github.com",
        language: "Node.js",
        stars: 3,
